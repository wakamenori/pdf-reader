...
# mermaid構文ルール
- ノードラベルは必ず生のダブルクォート " で囲む。&quot; / &#34; などのHTMLエンティティで代用しないこと（レンダラによってはそのまま文字として表示される）
- 各ノード定義・接続は必ず1行で書く
- 有効なノード形状: 四角 A["text"]  角丸 A("text")  ひし形 A{"text"}  円柱 A[("text")]  六角形 A{{"text"}}
- 開き括弧と閉じ括弧を正確に対応させること
- ラベル文字列内に () [] {} <> " 等の特殊文字を含める場合は、必ずラベル全体をダブルクォートで囲むこと
  - 悪い例: A[(ラベル(補足))]
  - 良い例: A[("ラベル(補足)")]
- 括弧 ( ) [ ] のみ曖昧になる場合はHTMLエンティティで置換可: ( → &#40;  ) → &#41;  [ → &#91;  ] → &#93;（ダブルクォート " は置換しないこと）
- ラベルに L001: / L042: のような行番号プレフィックスを含めてはならない。元コードに残っている場合は必ず除去する
  - 悪い例: A[("L005: 証明書発行業務")]
  - 良い例: A[("証明書発行業務")]
`;
