...
def get_gemini_patch(md, image_path, model_name, max_retries, retry_backoff) -> tuple[Answer, float]:
    ...
    if not service_account_json:
        raise RuntimeError(
            "GOOGLE_SERVICE_ACCOUNT_CREDENTIALS_VERTEX_AI is not set in .env"
        )
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            print(
                f"[Gemini] エラー発生（{attempt}回目）: {e}. {wait_time}秒後にリトライします。"
            )
            time.sleep(wait_time)
    # 規定回数失敗した場合は例外を投げる
    raise RuntimeError(
        f"Gemini APIの呼び出しに{max_retries}回失敗しました: {last_exception}"
    )
